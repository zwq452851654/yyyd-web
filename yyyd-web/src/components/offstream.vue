<template>
  ...
  <div id="enterprise" class="col-md-12">
      ...
      <div class="modal-dialog" role="document">
        <div class="modal-content">
          <div class="modal-header">
            ...
            <h4 class="modal-title" v-if="isAdd">设备停运记录(新增页面)</h4>
            <h4 class="modal-title" v-if="isUpdata">设备停运记录(修改页面)</h4>
          </div>
          <div class="modal-body ml0">
            <form class="form-inline clearfix" @submit.prevent="addsubmit">
              ...
              <div class="form-group">
                <label><span class="pk">*</span>设备停运起始时间：</label>
                <el-date-picker
              ...
              <div class="form-group">
                <label>停运原因：</label>
                <input type="text"  class="form-control" v-model="user.Reason" />
              </div>
              <div class="form-group">
                <label>负责人：</label>
                <input type="text" class="form-control" v-model="user.Person" />
              </div>
              <div class="form-group">
                <tabel>附&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;件：</tabel>
                <!-- 新增以及修改 -->
    ...
    <el-table @selection-change="handleSelectionChange" :data="list" border style="width: 100%" :default-sort="{prop: 'date', order: 'descending'}" @select-all="selectAll" @select="selectAll" >
      ...
      <el-table-column prop="EquipmentName" label="设备名称" sortable width="170">
      </el-table-column>
      <el-table-column prop="Starttime" label="设备停运起始时间" sortable width="170">
      </el-table-column>
      <el-table-column prop="Reason" label="停运原因" width="160">
      </el-table-column>
      <el-table-column prop="Person" label="负责人" width="130">
      </el-table-column>
  ...
  export default {
    ...
    methods:{
      ...
      getOverview:function(id){
        ...
        //局部发送ajax
        this.$http.get('/api/psf?TableCode=D004&COCode='+id)
          .then(function(res){
      ...
      OverviewDelAll:function(arr){
        ...
        this.$http.post('/api/pd', {TableCode: 'D004',COCode:this.id,RecordCode: delId}, {emulateJSON: true}).then((res) => {
            console.log(delId)
          if (res.body.message === "successful") {
          if(delId.length > 0){
            this.del(delIdAll);
